...
Mediamark phases: NEW, REVIEW, ESCALATED, SOW and Scoping, CLIENT APPROVAL, BACKLOG, IN PROGRESS, COMMS TO CLIENT, RESOLVED, NOT APPROVED, CHANGE REQUEST ON HOLD
    ...
    def sync_assignees_to_dydx(self, source_card_id: str, board_type: str, target_phase: str = None, enable_move: bool = False, is_move_event: bool = False, source_card: Dict = None, blocking_lock: bool = False) -> Dict:
        ...
        try:
            ...
            # create_dydx_card_for_assignee acquires a per-assignee lock and
            # always re-checks for an existing card inside the lock, so
            # concurrent threads cannot create duplicates even if they both
            # reach this point with the same diff result.
